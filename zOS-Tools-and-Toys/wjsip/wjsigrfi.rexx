/* REXX */
/**********************************************************************/
/* WJSIGRFI:  show file interest registration counts by file system   */
/*                                                                    */
/*   Syntax:                                                          */
/*    wjsigrfi [-v]                                                   */
/*        -v shows vnode information for each file                    */
/*                                                                    */
/* PROPERTY OF IBM                                                    */
/* COPYRIGHT IBM CORP. 2013                                           */
/*                                                                    */
/* Bill Schoen    wjs@us.ibm.com  10/21/13                            */
/*  Change activity:                                                  */
/*                                                                    */
/**********************************************************************/
/* REXX */
...
do while gfs@<>0
   ...
   do while vfs@<>0
      ...
      if vfsavail=0 & vfsdead=0 & vfsperm=0 then
         ...
         rcount=0
         do while vnod@<>0
            rfib@=getstor(xadd(vnod@,'98'),4,kasid,fds)
            if rfib@<>0 then
               do
               vnodname=getstor(xadd(vnod@,'A0'),16,kasid,fds)
               vnodname=translate(x2c(vnodname),' ','00'x)
               if verbose then
                  call say 'vfs@'vfs@ 'vnod@'vnod@ vnodname vfsname
               count=count+1
               rcount=rcount+1
               end
         ...
         if rcount>0 then
            call say right(rcount,5) vfsname
         end
      vfs@=getstor(xadd(vfs@,'08'),4,kasid,fds)
   end
end
if count=0 then
   call say 'no file interest registrations found'
