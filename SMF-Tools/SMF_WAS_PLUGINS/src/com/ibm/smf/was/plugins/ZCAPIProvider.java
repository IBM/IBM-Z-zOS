...
import com.ibm.smf.format.types.SMFType120SubType11UserDataType103;
...
/**
 * Format the z/OS Connect API provider User Data fields
 */
public class ZCAPIProvider extends ZCAPI implements SMFFilter {
    
    private SmfPrintStream smf_printstream = null;
    private boolean header_written = false;

    public static final int ZCEE_API_PROVIDER_USER_DATA_TYPE = 103;
    ...
    public void processRecord(SmfRecord record) {
        ...
        if(!recordIsValid(rec, ZCEE_API_PROVIDER_USER_DATA_TYPE)){
            return;
        }
        ...
        LibertyRequestInfoSection requestInfo = rec.m_libertyRequestInfoSection;
        long startTimeUs = new BigInteger(ConversionUtilities.longByteArrayToHexString(requestInfo.m_startStck), 16).shiftRight(12).longValue();
        long endTimeUs = new BigInteger(ConversionUtilities.longByteArrayToHexString(requestInfo.m_endStck), 16).shiftRight(12).longValue();
        appendTimeAndCPUInfo(requestInfo, startTimeUs, endTimeUs, sbData, sbHeader);

        // Format the User Data information
        SMFType120SubType11UserDataType103 sec = (SMFType120SubType11UserDataType103) rec.m_userDataSection[0];
        sbData.append(",").append(sec.m_version);
        sbHeader.append(",SMF120UD103_VERSION");
        ...
        sbHeader.append(",SMF120UD103_TIME_ZC_ENTRY");
        ...
        sbData.append(",").append(formatStck(todFormat));
        sbHeader.append(",SMF120UD103_TIME_ZC_EXIT");

        sbData.append(",").append(zcEntryTimeUs - startTimeUs);
        sbHeader.append(",SMF120UD103_TIME_ZC_ENTRY-SM120BBW");

        sbData.append(",").append(zcExitTimeUs == 0 ? 0 : endTimeUs - zcExitTimeUs);
        sbHeader.append(",SM120BBX-SMF120UD103_TIME_ZC_EXIT");

        sbData.append(",").append(sec.m_requestID);
        sbHeader.append(",SMF120UD103_REQ_ID");

        sbData.append(",").append(bytesToHex(sec.m_trackingToken));
        sbHeader.append(",SMF120UD103_TRACKING_TOKEN");
